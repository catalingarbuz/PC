...
int read_rtable(char* file) {
	FILE *f;
	f = fopen(file, "r");
	DIE(f == NULL, "Failed to open file");
	char line[100];
	int i = 0;
	...
	rtable_size = i;
	fclose(f);
	fprintf(stderr, "Done parsing table.\n");
	return 0;
}
...
struct route_table_entry *get_best_route(__u32 dest_ip) {
	...
	for (int i = 0; i < rtable_size; i++) {
		if ((rtable[i].mask & dest_ip) == rtable[i].prefix )                         
			if (idx == -1 || ntohl(rtable[idx].mask) 
				                < ntohl(rtable[i].mask))
				idx = i;
	}
...
struct arp_entry* create_arp_entry(__u32 ip, uint8_t *mac){
